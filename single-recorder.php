...
pcntl_signal(SIGINT, function($signo) {
    ...
    // Convert raw audio to MP4 using ffmpeg and merge them
    if (file_exists("recorded_audio0.raw") && file_exists("recorded_audio1.raw")) {
        $command = "ffmpeg -f s16le -ar 44100 -ac 2 -i recorded_audio0.raw -f s16le -ar 44100 -ac 2 -i recorded_audio1.raw -filter_complex amix=inputs=2:duration=longest -c:a aac -b:a 128k recorded_audio.mp4";
        exec($command, $output, $return_var);

        if ($return_var === 0) {
            echo "Recording stopped and file saved as recorded_audio.mp4.\n";
            unlink("recorded_audio0.raw");
            unlink("recorded_audio1.raw");
        } else {
            echo "Error merging and converting audio to MP4.\n";
        }
    }
    exit;
});
